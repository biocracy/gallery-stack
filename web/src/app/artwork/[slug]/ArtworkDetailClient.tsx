        ...
        <main className="min-h-screen max-w-[1600px] mx-auto px-6 py-12 sm:px-12 lg:py-20 relative z-10">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-24 items-start">
                ...
                <div
                    ...
                    onClick={() => artwork.image && openLightbox(0)}
                >
                    {artwork.image ? (
                        <Image
                ...
                <div className="flex flex-col justify-center space-y-8">
                    <div>
                        ...
                        {artwork.artist && (
                            <Link
                                ...
                                className="inline-block mt-2 text-xl font-medium text-neutral-600 dark:text-neutral-300 hover:text-blue-600 dark:hover:text-blue-400 transition-colors"
                            >
                    ...
                    <div className="pt-8 border-t border-neutral-200 dark:border-neutral-800">
                        {artwork.sold ? (
                            <div className="inline-flex px-8 py-4 bg-neutral-200 dark:bg-neutral-800 text-neutral-500 dark:text-neutral-400 text-lg font-semibold rounded-full cursor-not-allowed">
                                Sold
                            </div>
                        ) : (
                            <Link
                                href={`/inquire?artwork=${encodeURIComponent(artwork.title)}`}
                                className="inline-block w-full sm:w-auto px-8 py-4 bg-black dark:bg-white text-white dark:text-black text-lg font-semibold rounded-full hover:opacity-90 transition-opacity text-center"
                            >
            ...
            {artwork.moreImages && artwork.moreImages.length > 0 && (
                <section className="mt-24">
                    <h2 className="text-3xl font-bold text-neutral-900 dark:text-white mb-12">
                        Gallery
                    </h2>
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                        {artwork.moreImages.map((img: any, index: number) => (
                            <div
                                key={index}
                                ...
                                onClick={() => openLightbox(artwork.image ? index + 1 : index)}
                            >
                                <Image
                                    src={urlFor(img)}
